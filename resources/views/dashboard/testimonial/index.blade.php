...
@section('container')
    ...
    <div class="w-full flex flex-col-reverse lg:flex-row justify-between items-center mb-4 gap-4">
        <form class="lg:w-[800px] w-full flex flex-col lg:flex-row gap-4" action="/dashboard/testimonial">
        ...
        @include('dashboard.testimonial.create')
    </div>
    {{-- End Modal --}}
            ...
            <tbody>
                @foreach ($testimonials as $testimonial)
                    <tr class="bg-white border-b  :bg-gray-800  :border-gray-700 hover:bg-gray-50  :hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4">
                            <img class="w-14 h-14" src="{{ asset('storage/' . $testimonial->image_testi) }}"
                                alt="">
                        </td>
                        <td class="px-6 py-4 text-right flex gap-1">
                            <button type="button"
                                ...
                                data-modal-toggle="authentication-modal-edit{{ $testimonial->id }}"
                                data-modal-target="authentication-modal-edit{{ $testimonial->id }}">
                            ...
                            @include('dashboard.testimonial.edit', ['testimonialId' => $testimonial->id])
                            <form id="deleteForm" action="/dashboard/testimonial/{{ $testimonial->id }}" method="post">
                                @method('delete')
                                @csrf
