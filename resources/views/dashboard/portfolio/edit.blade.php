<!-- Main modal -->
<div id="authentication-modal-edit{{ $portfolioId }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow  :bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t ">
                ...
                <button type="button"
                    ...
                    data-modal-hide="authentication-modal-edit{{ $portfolio->id }}">
            ...
            <div class="p-4 md:p-5">
                <form class="space-y-4" action="/dashboard/portfolio/{{ $portfolio->id }}" method="post"
                    ...
                    @enderror
                            ...
                            placeholder="Car Image" required value="{{ old('name', $portfolio->name) }}">
                        @error('name')
                    ...
                    <div>
                        <label for="altimg"
                            class="block mb-2 text-sm font-medium text-gray-900 text-start  :text-white">Alt
                            Image</label>
                        <input
                            @error('altimg')
                    class="bg-gray-50 border border-red-600 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    @enderror
                            ...
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                            placeholder="carimage" value="{{ old('altimg', $portfolio->altimg) }}" required>
                        @error('altimg')
                            <p class="mt-2 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="category"
                            class="block mb-2 text-sm font-medium text-gray-900 text-start  :text-white">Select
                            an
                            category</label>
                        <select
                            @error('category_id')
                            ...
                            @foreach ($categories as $category)
                                @if (old('category_id', $portfolio->category_id) == $category->id)
                    ...
                    <div>
                        ...
                        @if ($portfolio->image)
                            <img src="{{ asset('storage/' . $portfolio->image) }}" class="img-preview w-56 my-2">
                        @else
                            <img src="" class="img-preview w-56 my-2">
                        @endif
                        <input class="block w-full text-[16px] text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50" id="image" type="file" name="image" onChange="previewImage()">
                        @error('image')
                            <p class="mt-2 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            placeholder="Description">{{ old('description', $portfolio->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
